...
void Aimbot::DoubleTapExploit() {
	if (!g_aimbot.m_double_tap && g_cl.m_charged) {
		g_cl.m_charge_timer = 0;
	...
	if (!g_cl.m_charged) {
		if (g_cl.m_charge_timer > game::TIME_TO_TICKS(.5)) {
			g_cl.m_tick_to_recharge = 16;
		}
		else {
	...
	if (g_cl.m_cmd->m_buttons & IN_ATTACK && g_cl.m_weapon_fire && g_cl.m_charged) {
		g_cl.m_charge_timer = 0;
		g_cl.m_tick_to_shift = 16;
		*g_cl.m_packet = false;
	}
	if (!g_cl.m_charged) {
		g_cl.m_charged_ticks = 0;
	}
}
...
void FixVelocity(Player* player, LagRecord* record, LagRecord* previous, float max_speed)
{
	if (!previous)
	{
		if (record->m_layers[6].m_playback_rate > 0.0f && record->m_layers[6].m_weight != 0.f && record->m_velocity.length() > 0.1f)
		{
			float velocity_scale = max_speed;
			if (record->m_flags & 6)
				velocity_scale *= 0.34f;
			else if (player->m_bIsWalking())
				velocity_scale *= 0.52f;

			float velocity_length = record->m_velocity.length();
			if (velocity_length > 0.0f)
				record->m_velocity *= velocity_scale / velocity_length;
		}
		else
		{
	...
	if ((player->m_fEffects() & 8) != 0 || player->m_ubEFNoInterpParity() != player->m_ubEFNoInterpParityOld())
	{
		record->m_velocity.clear();
	...
	bool is_jumping = !(record->m_flags & FL_ONGROUND && previous->m_flags & FL_ONGROUND);

	if (record->m_lag > 1)
	{
		record->m_velocity.clear();
		vec3_t origin_delta = record->m_origin - previous->m_origin;
		...
		float sqrt_delta = origin_delta.length_2d_sqr();
		...
		if (is_jumping)
		{
			...
			{
				float max_speed_bunnyhop = player->m_flMaxspeed() * 1.1f;
				float speed = record->m_velocity.length();
				if (max_speed_bunnyhop > 0.f && speed > max_speed_bunnyhop)
					record->m_velocity *= (max_speed_bunnyhop / speed);
			}
	...
	if (!record->m_fake_walk)
	{
		if (record->m_anim_velocity.length_2d() > 0 && (record->m_flags & FL_ONGROUND))
		{
			...
			if (!is_jumping && record->m_layers[11].m_weight > 0.0f && record->m_layers[11].m_weight < 1.0f && record->m_layers[11].m_playback_rate == previous->m_layers[11].m_playback_rate)
			{
				float flAnimModifier = 0.35f * (1.0f - record->m_layers[11].m_weight);
			...
			if (anim_speed > 0.0f)
			{
				float anim_speed_scale = anim_speed / record->m_anim_velocity.length_2d();
				record->m_anim_velocity.x *= anim_speed_scale;
				record->m_anim_velocity.y *= anim_speed_scale;
			}
		}
	}
	else
...
void AimPlayer::UpdateAnimations(LagRecord* record)
{
	...
	// backup stuff that we do not want to modify.
	...
	if (record->m_lag > 0 && record->m_lag < 19 && m_records.size() >= 2)
	{
	...
	if (record->m_lag > 1 && !bot)
	{
		...
		// we need at least 2 updates/records
		// to fix these issues.
		if (m_records.size() >= 2)
		{
			if (previous && !previous->dormant())
			{
				...
				// been on ground for 2 consecutive ticks? fuck yeah.
	...
	if (record->m_eye_angles.z != 0.f)
	{
		record->m_eye_angles.z = 0.f;
	}
	...
	// fix animating in the same frame.
	...
	state->m_goal_feet_yaw = record->m_body;
	...
	// also don't restore the render angles which indicate the model rotation.
...
void AimPlayer::SetupHitboxes(LagRecord* record, bool history) {
	...
	// prefer, in air.
	if (g_menu.main.aimbot.baim1.get(5) && (m_missed_shots >= g_menu.main.aimbot.misses.get()))
		m_hitboxes.push_back({ HITBOX_BODY, HitscanMode::PREFER });
	...
	// only, in air.
	if (g_menu.main.aimbot.baim2.get(4) && (m_missed_shots >= g_menu.main.aimbot.misses.get())) {
		only = true;
		m_hitboxes.push_back({ HITBOX_BODY, HitscanMode::PREFER });
	}
...
void Aimbot::find() {
	...
	for (const auto& t : m_targets) {
		...
		if (g_menu.main.aimbot.lagfix.get() && (g_lagcomp.StartPrediction(t) || t->m_records.front().get()->m_invalid || g_csgo.m_cvar->FindVar(HASH("cl_lagcompensation"))->GetInt() == 0)) {
...
bool AimPlayer::GetBestAimPosition(vec3_t& aim, float& damage, LagRecord* record) {
	...
	if (g_cl.m_weapon_id == ZEUS) {
		...
		if (g_aimbot.m_damage_override) {
			dmg = g_menu.main.aimbot.damageoverrideamount.get();
			pendmg = g_menu.main.aimbot.damageoverrideamount.get();

			if (g_menu.main.aimbot.damageoverrideamount.get() > 100) {
			...
			pendmg = g_menu.main.aimbot.minimal_damage.get();

			if (g_menu.main.aimbot.minimal_damage.get() > 100) {
				dmg = std::ceil((dmg / 100.f) * hp);
				pendmg = std::ceil((pendmg / 100.f) * hp);
			}
		}
